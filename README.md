...
working directory in a `.service` script, when you launch the server as a service. 
...
| max_chunk_size_kilo | number | max chunk size in kilobytes (0 no chunks, the complete load is in the memory)  | 0 |
...
How it can be done on Linux, is described in [the document](https://github.com/vernisaz/simcom/blob/master/doc/PIs.md#run-simhttp-as-a-service).
...
and CGI websocket endpoints. It doesn't require to rebuild the server for every application. For example, I implemented
[TOTP](https://github.com/vernisaz/simtotp) with web interface,
