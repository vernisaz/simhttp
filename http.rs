...
fn handle_connection(mut stream: &TcpStream, close_connection: bool) -> io::Result<()> {
    ...
    if method == "GET" || method == "POST" {
                ...
                if cgi {
                    ...
                    let stdout_thread = thread::spawn(move || -> io::Result<()> {
                        ...
                        let mut try_process = || -> io::Result<()> {
                            ...
                            if !no_headers {
                                ...
                                // && rc == '\r'
                                {
                                    if let Some(b) = line.as_bytes().last()
                                        && *b == b'\r'
                                    {
                                        line.pop();
                                    }
                                    ...
                                    return Err(io::Error::other(format!(
                                        "empty line or improper line separator in {line}"
                                    )));
                                }
                            ...
                            'chunked: {
                                if *chunk_size > 0 {
                                    ...
                                    if gzip_allowed {
                                        ...
                                        if len as u64 > GZIP_LOW_BOUNDARY_THRESHOLD {
                                            ...
                                            if len == 0 {
                                                ...
                                                let max_sz = compressor
                                                    .gzip_compress_bound(backup_buffer.len());
                                                ...
                                                write!(out_stream, "{actual_sz:x}\r\n")?;
                                                out_stream.write_all(&compressed_data)?;
                                                write!(out_stream, "\r\n0\r\n\r\n")?;
                                                //eprintln!("written gzip of {} in {actual_sz}", backup_buffer.len());
                                                out_stream.flush()?;
                                                break 'chunked;
                                            }
                                        }
                                    ...
                                    if gzip_allowed && written > GZIP_LOW_BOUNDARY_THRESHOLD {
                                        let mut compressor =
                                            Compressor::new(CompressionLvl::default());
                                        let max_sz = compressor.gzip_compress_bound(buffer.len());
                                        let mut compressed_data = vec![0; max_sz];
                                        let actual_sz = compressor
                    ...
                    if since > 0
                        && modified
                            ...
                            < since
                    {
                        let response = format!("{protocol} 304 {}\r\n\r\n", response_message(304));
                        stream.write_all(response.as_bytes())?;
                        // log
                        LOGGER
                            .lock()
        ...
        // PUT DELETE HEAD TRACE OPTIONS PATCH CONNECT
        // unsupported method
...
fn read_mapping(mapping: &Vec<JsonData>) -> Vec<Mapping> {
    ...
    for e in mapping {
        ...
        let Some(Text(path)) = e.get("path") else {
            continue;
        };

        let Some(Text(trans)) = e.get("translated") else {
            continue;
        };
        ...
        let no_headers = e
            .get("headerless")
            .map(|val| if let Bool(val) = val { val } else { &false })
            .unwrap_or(&false);
        ...
        res.push(Mapping {
            web_path: if *websocket || path.ends_with("/") {
                path.to_string()
            } else {
                path.to_string() + "/"
            },
            ...
            options: if options_res.is_empty() {
                None
            } else {
                Some(options_res)
            },
...
fn encode_block(input: &[u8]) -> Vec<u8> {
    // TODO add param - start, mid and the last block
    let len = input.len();
...
fn decode_block(input: &mut [u8]) -> Result<DecodedBlockData, String> {
    ...
    let (len, mut shift) = match input[1] & 0x7f {
        len @ 0..=125 => (len as usize, 2_usize),
        126 => {
            if buf_len >= 4 {
                (
                    u16::from_be_bytes(input[2..4].try_into().unwrap()) as usize,
                    4_usize,
                )
            } else {
                (0usize, buf_len)
        ...
        127 => {
            if buf_len >= 10 {
                (
                    u64::from_be_bytes(input[2..10].try_into().unwrap()) as usize,
                    10_usize,
                )
            } else {
                (0usize, buf_len)
    ...
    if buf_len < shift + 4 {
        // request to get more block data
        return Ok((res, op, last, 0, [0u8; 4], 0, false));
    }
    let mut curr_mask = 0;
    let mask;
    if masked {
        // redundant if because else branch is never triggered
        mask = [
            input[shift],
            input[shift + 1],
            input[shift + 2],
